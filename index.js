	...
	if (code.length == 6 && !isNaN(code)) {
		...
		.then((data) => {
			...
			if (data.responses[0].models.gameInstance.length > 0) {
				...
				this.log('Obtaining site data, paste it in the input above.', '#fff');
			} else {
...
Live.prototype.interval = function () {
	if (!this.paused && this.data && Object.keys(this.data).length > 0 && window.location.href.indexOf('quizlet') > 0 && window.location.href.indexOf('live') > 0) {
		if (document.getElementById('liveAnswerPhrase') && document.getElementsByClassName('StudentPrompt-inner')[0] && document.getElementsByClassName('StudentTerm is-clickable can-beClicked').length) {
			...
			this.data.terms.filter((term) => {
				if (term.term.toLowerCase() == question) return true;
				else if (term.definition.toLowerCase() == question) return true;
			}).forEach(function(word) {
				if (word.term.toLowerCase() == question) document.getElementById('liveAnswerPhrase').textContent = word.definition;
				else document.getElementById('liveAnswerPhrase').textContent = word.term;

				for (let i = 0; i < options.length; i++) {
					let option = options[i].innerText.toLowerCase().trim();
					if (word.definition.toLowerCase() == option) click(options[i]);
					else if (word.term.toLowerCase() == option) click(options[i]);
				}
			});
		}
	}
